...
namespace AzyleeCC.Core.IOUtils.FileUtils
{
    ...
    public class FileEncryptTool
    {
        ...
        /// -404：未知错误，操作失败
        /// </returns>
        public static int Encrypt(string srcFile, string dstFile, string password, Action<object, long, long> action = null, object sender = null, bool overwrite = true)
        {
                ...
                using (FileStream fsWrite = new FileStream(dstFile, FileMode.Create))
                {
                    try
                    {
                        ...
                        while ((readCount = fsRead.Read(buffer, 0, buffer.Length)) > 0)
                        {
                            ...
                            action?.Invoke(sender, fsRead.Position, fsRead.Length);
                        }
        ...
        /// -404：未知错误，操作失败
        /// </returns>
        public static int Decrypt(string srcFile, string dstFile, string password, Action<object, long, long> action = null, object sender = null, bool overwrite = true)
        {
            DateTime beginTime = DateTime.Now;
