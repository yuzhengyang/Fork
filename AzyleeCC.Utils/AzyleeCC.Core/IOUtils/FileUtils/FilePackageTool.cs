...
namespace AzyleeCC.Core.IOUtils.FileUtils
{
    ...
    public class FilePackageTool
    {
        ...
        /// -404;//未知错误，操作失败
        /// </returns>
        public static int Pack(string srcPath, string dstFile, Action<object, long, long> action = null, object sender = null, bool overwrite = true)
        {
                        ...
                        files.ForEach(x =>
                        {
                            ...
                            using (FileStream fsRead = new FileStream(DirTool.Combine(srcPath, x.Path, x.Name), FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                            {
                                ...
                                while ((readCount = fsRead.Read(buffer, 0, buffer.Length)) > 0)
                                {
                                    ...
                                    action?.Invoke(sender, allfilesize - surplusfilesize, allfilesize);
                                }
                            }
                        });
                    }
        ...
        /// -99;//未知错误，操作失败
        /// </returns>
        public static int Unpack(string srcFile, string dstPath, Action<object, long, long> action = null, object sender = null, bool overwrite = true)
        {
            DateTime beginTime = DateTime.Now;
                                    ...
                                    try
                                    {
                                        using (FileStream fsWrite = new FileStream(DirTool.Combine(dstPath, x.Path, x.Name), FileMode.Create))
                                        {
                                            ...
                                            while (size > FileBuffer)
                                            {
                                                ...
                                                size -= readCount;
                                                current += readCount;
                                                action?.Invoke(sender, current, allfilesize);
                                            }
                                            if (size <= FileBuffer)
                                            {
